 ...
 * @author    : jianglong <jianglong@qiyi.com>
 * @date      :   14-11-27
 */
...
class SMTPClient
{
	 ...
	 */
	public function setMail($to, $subject, $message, $contentType = "text/html", $charset = "UTF-8") {
		if (is_array($to)) {
			$this->to = $to;
		} else {
	...
	public function sendMail() {
		...
		try {
			...
			if ($this->_debug) {
				...
				if (!($socket = fsockopen(($this->security ? $this->security."://" : "").$this->smtpHost, $this->smtpPort, $errno, $errstr, 15))) {
			...
			if ($this->contentType != "text/plain") {
				...
				$multiPartMessage .= "Content-Type: text/plain; charset=\"".$this->charset."\""."\r\n";
				$multiPartMessage .= "Content-Transfer-Encoding: "."quoted-printable"."\r\n";
				$multiPartMessage .= "\r\n";
				$multiPartMessage .= quoted_printable_encode(strip_tags($this->message))."\r\n";
				$multiPartMessage .= "\r\n";
			}
			//Raw text mail version
			$multiPartMessage .= "--".$mimeBoundary."\r\n";
			$multiPartMessage .= "Content-Type: ".$this->contentType."; charset=\"".$this->charset."\""."\r\n";
			$multiPartMessage .= "Content-Transfer-Encoding: quoted-printable"."\r\n";
			$multiPartMessage .= "\r\n";
			...
			fwrite($socket, "To: <".implode(">, <", $this->to).">"."\r\n");
	...
	private function _serverRespondedAsExpected($socket, $expectedStatusCode) {
		...
		while (substr($serverResponse, 3, 1) != " ") {
			...
			//			echo $serverResponse;
			if (!($serverResponse)) {
				throw new Exception("Couldn\'t get mail server response codes."." Please contact an administrator.");
			}
		}
